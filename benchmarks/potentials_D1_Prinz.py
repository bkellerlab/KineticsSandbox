...
if test_F:
    ...
    # plot
    color = plt.cm.viridis(0)  # Normalize a to be in [0, 1]
    plt.plot(x, my_potential.force(x, h), color=color)
...
if test_H:
    ...
    # plot
    color = plt.cm.viridis(0)  # Normalize a to be in [0, 1]
    plt.plot(x, my_potential.hessian(x, h), color=color)
...
if test_TS:
    ...
    # Numerical solution
    TS1_num = my_potential.TS(-0.75, -0.25)
    ...
    TS_data = [
        ...
        [
            "Energy",
            my_potential.potential(TS1_num),
            my_potential.potential(TS2_num),
            my_potential.potential(TS3_num),
        ],
        [
            "Force",
            my_potential.force(TS1_num, h),
            my_potential.force(TS2_num, h),
            my_potential.force(TS3_num, h),
        ],
        [
            "Hessian",
            my_potential.hessian(TS1_num, h),
            my_potential.hessian(TS2_num, h),
            my_potential.hessian(TS3_num, h),
        ],
    ]
